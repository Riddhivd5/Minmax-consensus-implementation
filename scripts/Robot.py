...
rospy.init_node("Controller{num}".format(num=index), anonymous=True) # initialize ros node

file_path = __file__
dir_path = file_path[: (len(file_path) - len("control.py"))] # get into the direcory
...
def handler(signum, frame):
    ...
    for i in range(num_agent):
        np.save(dir_path + "numpy_array/Position-X-Robot-{num}".format(num=i+1),robot[i].position_x)
        np.save(dir_path + "numpy_array/Position-Y-Robot-{num}".format(num=i+1),robot[i].position_y)
    plt.clf()
    ...
    if data["Plots"]["Velocity"]:
        ...
        if(data["Plots"]["Show_plot"]) : plt.show()
        for i in range(num_agent):
            np.save(dir_path + "numpy_array/Velocity-X-Robot-{num}".format(num=i+1),robot[i].velocity_x)
        plt.clf()
        ...
        if(data["Plots"]["Show_plot"]) : plt.show()
        for i in range(num_agent):
            np.save(dir_path + "numpy_array/Velocity-Y-Robot-{num}".format(num=i+1),robot[i].velocity_y)
        plt.clf()
